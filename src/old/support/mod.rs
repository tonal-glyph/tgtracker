...
pub fn gui(ui: &mut conrod::UiCell, ids: &Ids, app: &mut DemoApp) {
    ...
    for _press in widget::Button::new()
        ...
        .set(ids.button, ui)
    {
        let x = nannou::rand::random::<conrod::Scalar>() * (max_x - min_x) - max_x;
        let y = nannou::rand::random::<conrod::Scalar>() * (max_y - min_y) - max_y;
        app.ball_xy = [x, y];
    }
...
impl EventLoop {
    ...
    pub fn next(
        &mut self,
        events_loop: &mut nannou::glium::glutin::EventsLoop,
    ) -> Vec<nannou::glium::glutin::Event> {
        ...
        if events.is_empty() && !self.ui_needs_update {
            events_loop.run_forever(|event| {
                events.push(event);
                nannou::glium::glutin::ControlFlow::Break
            });
        }
